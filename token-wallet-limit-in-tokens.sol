...
/**
 * It is example of a Wallet Limit Tokens in Tokens from VladimirGav
...
contract SwapBlock is Ownable {
    ...
    function setTokensWalletLimit(uint256 _tokensWalletLimit) public onlyOwner {
        if(_tokensWalletLimit>0){
            require(_tokensWalletLimit >= minTokensWalletLimit, "_tokensWalletLimit < minTokensWalletLimit");
        }

        tokensWalletLimit = _tokensWalletLimit;
    }
...
contract SimpleToken is Context, Ownable, IERC20, SwapBlock {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        // Check TokensWalletLimit
        if(owner() != recipient && !addressesLiquidity[recipient] && SwapBlock.getTokensWalletLimit()>0){
            require(_balances[recipient].add(amount) <= SwapBlock.getTokensWalletLimit(), "Transfer PercentsWalletLimit");
        }
